...
@Slf4j
public class NginxHttpServerDownloadHandler extends BaseHttpServerHandler {

    private final DownloadConfig downloadConfig;

    private final DownloadContext downloadContext = new DownloadContext();

    public NginxHttpServerDownloadHandler(DownloadConfig downloadConfig, Semaphore concurrentLimit) {
        ...
        if (msg instanceof HttpRequest) {
            ...
            downloadConfig.getAsyncLoadFile().asyncLoad(downloadContext.getId(), (localFile, ext, len, needRelease) -> {
                if (StringUtils.isEmpty(localFile)) {
                    ...
                    writeResponse(ctx.channel(), localFile, ext);
                }
                reset();
            });
        }
...
//        boolean keepAlive = false;
    ...
    private void writeResponse(Channel channel, String targetUrl, String ext) {
        FullHttpResponse response = new DefaultFullHttpResponse(
        ...
        if ("application/octet-stream".equals(contentType)) {
            String attachmentName = String.format("attachment;filename=%s", FilenameUtils.getName(targetUrl));
            headers.set(HttpHeaderNames.CONTENT_DISPOSITION, attachmentName);
        }


        if (targetUrl.startsWith("/")) {
            headers.set("X-Accel-Redirect", targetUrl);
        } else {
            headers.set("X-Reproxy-URL", targetUrl);
            headers.set("X-Accel-Redirect", "/reproxy");
        }
